...
class Recon():
    ...
    def run_recon(self, target, commands):
        ...
        Path(log_dir).mkdir(exist_ok=True, parents=True)
        for command, filename in commands:
            if command == HCITOOL_INFO[0]:
                # Retry up to 5 times until we get LMP version data
                for attempt in range(5):
                    try:
                        ...
                        if "LMP Version" in output:
                            ...
                            f.write(output)
                            f.close()
                            print(f"Got full LMP data on attempt {attempt + 1}")
                            break
                        else:
                    ...
                    # All retries exhausted, save whatever we got last
                    print("Could not get full LMP data after 5 attempts, saving partial data")
