...
BTCTL_AGENT_NINO = "bluetoothctl agent NoInputNoOutput"
...
def check_nino(target):
    ...
    try:
        ...
        #     report_error("Couldn't remove the device before testing")
            # 
            # report_error("Error removing the device")
            #return
        ...
        if output is not None: # and target in output:
            ...
            logging.info("nino_check.py -> Target device not found during scan")
            report_error("Target device not found during scan")
